#!/usr/bin/env python3
"""Retrieval quality evaluation experiment.
...
class RealEmbedder:
    """Real BGE embedder using sentence-transformers (BAAI/bge-small-zh-v1.5)."""
    ...
    def _load(self):
        if self._e is None:
            ...
            self._e = LocalEmbedder(model_name="BAAI/bge-small-zh-v1.5", device="cpu")
            self._e._ensure_loaded()
...
def make_queries(documents: list[dict]) -> list[dict]:
    ...
    for doc in documents:
        # query1: first 8-15 chars of title (exact match)
        title = doc["title"]
        q1_len = random.randint(8, min(15, len(title)))
        q1 = title[:q1_len]

        # query2: random 10-20 char snippet from body (fuzzy match)
        body = doc["body"]
...
def _rank_of(results: list[SearchResult], source: str) -> int:
    """Return 0-based rank of *source*, or -1 if not found."""
...
def baseline(embedder: RealEmbedder, store: VectorStore, query: str, top_k: int) -> list[SearchResult]:
    """Pure vector search."""
...
def hybrid_fuse(
    ...
    """Vector search → BM25 fusion → return *candidates_count* results."""
    qvec = embedder.embed_query(query)
...
def hybrid(
    ...
    """Hybrid retrieval (BM25 + vector fusion) → top_k."""
...
def main() -> None:
    ...
    print("=" * 56)
    print("  检索质量统计测试实验 (Retrieval Quality Evaluation)")
    print("=" * 56)
    print(f"  文档数: {NUM_DOCS}, 查询数: {NUM_DOCS * 2}, top_k: {TOP_K}")
    print(f"  hybrid_alpha: {HYBRID_ALPHA}, embedding_dim: {EMBEDDING_DIM}")
    print()
    ...
    print(f"  -> 示例正文: {docs[0]['body'][:60]}...")
    print()

    # 2. Build FAISS vector store
    print("[2/5] 构建向量索引 ...")
    embedder = RealEmbedder(dim=EMBEDDING_DIM)
    ...
    print(f"  -> 示例精确查询: q0={queries[0]['query']!r}")
    print(f"  -> 示例模糊查询: q1={queries[1]['query']!r}")
    print()

    # 4. Try to load cross-encoder reranker
    print("[4/5] 尝试加载重排序模型 ...")
    ...
    try:
        ...
        # Trigger lazy load with a tiny test call
        test_q = "测试"
    ...
    for idx, q in enumerate(queries):
        ...
        if reranker is not None:
            # Get hybrid fused candidates (40 docs) as input for reranker
    ...
    for s in strategies:
        ...
        exact_rr = sum(
            1.0 / (e[f"{s}_rank"] + 1) for e in per_query if e["query_type"] == "exact" and e[f"{s}_rank"] >= 0
        )
        exact_mrr = exact_rr / n_exact
        fuzzy_rr = sum(
            1.0 / (e[f"{s}_rank"] + 1) for e in per_query if e["query_type"] == "fuzzy" and e[f"{s}_rank"] >= 0
        )
