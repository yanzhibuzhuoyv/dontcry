...
NUM_DOCS = 250
TOP_K = 10
HYBRID_CANDIDATES = TOP_K * 2  # 20
HYBRID_ALPHA = 0.5
EMBEDDING_MODEL = "BAAI/bge-small-zh-v1.5"
RERANKER_MODEL = r"C:\Users\32202\Desktop\99999\models\BAAI_bge-reranker-base"
RERANKER_TIMEOUT = 600  # local load, generous
# Rerank only sees top_k hybrid candidates (not all) for CPU speed.
# 1000 queries × 10 pairs × ~0.05s ≈ 500s (within 10-min timeout).
RERANK_CANDIDATES = TOP_K  # 10
RESULTS_PATH = Path(__file__).resolve().parent / "results_v5.json"
...
def _try_create_reranker() -> tuple:
    ...
    Uses a direct load (no test prediction, no alarm — local path).
    """
...
def main() -> None:
    ...
    for idx, q in enumerate(queries):
        ...
        # rerank (if loaded) — only rerank top hybrid candidates for CPU speed
        if reranker is not None:
            try:
                fused_cand = hybrid(embedder, store, q["query"], RERANK_CANDIDATES, HYBRID_CANDIDATES, HYBRID_ALPHA)
                reranked = reranker.rerank(q["query"], fused_cand, TOP_K)
                entry["rerank_rank"] = _rank_of(reranked, q["relevant_source"])
            except Exception as rexc:
                print(f"  [ERROR] rerank failed for query {idx}: {rexc}")
                traceback.print_exc()
                entry["rerank_rank"] = -1
